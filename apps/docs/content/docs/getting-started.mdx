...
<Callout type="info">
  **🔬 Ready to explore?** Test your API access and explore available query
  types in our [Interactive API Playground](/api). Perfect for understanding
  data structures before building integrations!
</Callout>
...
<Callout type="info">
  **Best Practice**: Disable tracking in development to avoid polluting your
...
<button
  onclick="trackFeatureUsed('export', {tier: 'premium', category: 'data'})"
>
  Export Data
</button>`}
/>
...
<Callout type="info">
  **Property Analytics**: Your custom event properties are automatically
  ...
  distributions and value percentages in an interactive tree structure.
</Callout>
